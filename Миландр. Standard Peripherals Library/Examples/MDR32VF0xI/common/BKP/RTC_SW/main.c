...
/** @addtogroup MDR32VF0xI_BKP_RTC_SW MDR32VF0xI_BKP_RTC_SW
 * @{
 */
...
/**
 * @brief  BKP RTC interrupt handler.
 * @param  None.
 * @return None.
 */
void BKP_IRQHandler(void)
{
    for (uint32_t Cnt = 0; Cnt < 4; Cnt++) {
        ...
        DELAY_WaitMs(LED_TOGGLE_MS);
    }
    BKP_RTC_WUT_SetAlarm(BKP_RTC_WUT_GetCounter() + BKP_WAKEUP_S - 1);
    BKP_RTC_ClearFlags(BKP_RTC_FLAG_WUTF);
    if (BKP_SW_GetPowerStatus()) {
        BKP_SW_DisablePower();
    }
}
...
void BKP_Config(void)
{
    ...
    while (BKP_CLK_GetFlagStatus(BKP_FLAG_LSERDY) == RESET) { }
    BKP_RTC_SetClkSource(BKP_RTC_CLK_SRC_LSE);
    BKP_RTC_WaitClkSwitch(BKP_RTC_CLK_SRC_LSE);
    BKP_RTC_Reset(DISABLE);
    BKP_RTC_SetPrescaler(LSE_FREQUENCY_Hz - 1);
    BKP_RTC_WUT_SetClkSource(BKP_RTC_WUT_CLK_SRC_SEC_CLK);
    BKP_RTC_WUT_SetAlarm(BKP_WAKEUP_S - 1);
...
/** @} */ /* End of group MDR32VF0xI_BKP_RTC_SW */
